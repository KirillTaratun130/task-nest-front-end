...
class TimerService {
    ...
    async getTodaySession() {
        const response = await axiosWithAuth.get<ITimerSessionResponse>(`${this.BASE_URL}/today`)
        return response
    }

    async createSession() {
        const response = await axiosWithAuth.post<ITimerSessionResponse>(this.BASE_URL)
        return response
    }
